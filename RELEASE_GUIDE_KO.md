...
공개 배포 기준으로 하루에 노출하는 최신 버전은 하나만 둡니다.

- 같은 날짜에 나온 문서 수정, 기본값 수정, 패키지 재빌드는 그날의 배포 버전에 흡수합니다.
- 팀 공유 사이트의 최신 다운로드, 최신 업데이트 노트, `VERSION`, `.deb` 파일명은 모두 같은 버전을 가리켜야 합니다.
- 같은 날짜에 실수로 새 패치 버전을 만들었다면 공개 사이트에서는 제거하고, 기존 당일 배포 버전으로 다시 빌드해 정리합니다.
- 다음 날짜에 새로 배포하거나 이미 팀 전체 설치가 끝난 뒤 구분이 필요한 수정만 새 패치 버전으로 올립니다.
...
기능을 지우거나 패키지에 포함되는 파일 구성이 바뀌면 보통 패치 버전을 올립니다. 단, 같은 날짜의 배포 정리 중 발견된 수정은 위의 하루 한 버전 원칙에 따라 그날의 공개 배포 버전에 합쳐서 다시 빌드합니다.
